...
class Mastodon:
    ...
    def log_in(self, username=None, password=None,
        ...
        if requested_scopes != received_scopes:
            raise MastodonAPIError('Granted scopes "' + received_scopes +
    ...
    def media_post(self, media_file, mime_type=None):
        ...
        if mime_type is None:
            raise MastodonIllegalArgumentError('Could not tell mime type or data passed directly without mime type.')

        random_suffix = ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(10))
        file_name = "mastodonpyupload_" + str(time.time()) + "_" + \
                    str(random_suffix) + mimetypes.guess_extension(mime_type)
    ...
    def __api_request(self, method, endpoint, params=dict(), files=dict(), do_ratelimiting=True):
        """
        ...
        while not request_complete:
            ...
            try:
                ...
                if method == 'DELETE':
                    response_object = requests.delete(
                        self.api_base_url + endpoint,
                        data=params,
                        headers=headers,
                        files=files,
